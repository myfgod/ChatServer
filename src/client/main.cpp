...
//聊天客户端程序实现，main线程做发送线程，子线程做接收线程
int main(int argc,char ** argv)
{
    ...
    for(;;)
    {
        ...
        while (true)
        { // 使用无限循环来不断要求用户输入，直到获得有效输入
            std::cout << "Please enter an integer: ";
            std::getline(std::cin, input); // 读取一整行输入

            try
            {
                choice = std::stoi(input); // 尝试将字符串转换为 int
    ...
    else
    {
        ...
        if(responsejs.contains("groups"))
        {
            ...
            for(string groupstr:vec_1)
            {
                ...
                vector<string> vec_2 = grpjs["users"];
                for(string &userstr:vec_2)
                {
                    groupUser user;
...
void readTaskHandler(int clientfd)
{
    for(;;)
    {
        ...
        //接收ChatServer转发的消息，反序列化生成json对象
